(function () {
    ...
    function cargarCatalogos() {
        var p1 = formas ? Promise.resolve(formas)
            : fetch("/recepcionista/api/comprobante/formas-pago").then(function (r) {
                return r.json();
            }).then(function (d) {
                formas = d;
                return d;
        ...
        var p2 = tipos ? Promise.resolve(tipos)
            : fetch("/recepcionista/api/comprobante/tipos-comprobante").then(function (r) {
                return r.json();
            }).then(function (d) {
                tipos = d;
                return d;
    ...
    function abrirModal(codigo) {
        ...
        fetch("/recepcionista/api/comprobante/resumen/" + encodeURIComponent(codigo))
            ...
            .then(function (res) {
                if (!res.ok) {
                    throw new Error(res.body.mensaje || "No se pudo cargar la reserva");
                }
                resumenActual = res.body;
                if (resumenActual.yaPagada) {
                    alert("Esta reserva ya tiene un comprobante emitido.");
                    return;
                }
                return cargarCatalogos().then(function () {
                    pintarResumen();
                    el("modal-pago").style.display = "flex";
                });
            })
            .catch(function (e) {
                alert(e.message);
    ...
    function pintarResumen() {
        ...
        // formas
        var selForma = el("pago-forma");
        selForma.innerHTML = "";
        (formas || []).forEach(function (f) {
            var o = document.createElement("option");
            o.value = f.idFormaPago;
            o.textContent = f.nombrePago;
            selForma.appendChild(o);
        });
    }

    function cerrar() {
        el("modal-pago").style.display = "none";
        codigoActual = null;
    }
    ...
    function emitir() {
        ...
        if (!tipo) {
            el("e-pago-tipo").textContent = "Selecciona un tipo de comprobante.";
            return;
        }
        var idForma = el("pago-forma").value ? parseInt(el("pago-forma").value, 10) : null;
        var headers = {"Content-Type": "application/json"};
        headers[csrfHeader] = csrfToken;
        var btn = el("pago-emitir");
        btn.disabled = true;
    ...
    function imprimir(codigo) {
        fetch("/recepcionista/api/comprobante/vigente/" + encodeURIComponent(codigo))
            ...
            .then(function (res) {
                if (!res.ok || !res.idComprobante) {
                    alert("No se encontró un comprobante emitido para esta reserva.");
                    return;
                }
