(function () {
    ...
    var SUF = {
        nombres: "nombres", apellidos: "apellidos", fechaNacimiento: "fnac",
    ...
    function habDe(id) {
        return disponibles.find(function (h) {
            return h.id === id;
        });
    }
    ...
    function getCliente() {
        return {
            nombres: val("cli-nombres"), apellidos: val("cli-apellidos"), fechaNacimiento: val("cli-fnac") || null,
    ...
    function getModal() {
        return {
            nombres: val("hu-nombres"), apellidos: val("hu-apellidos"), fechaNacimiento: val("hu-fnac") || null,
    ...
    function personasExcepto(roomIdx, guestIdx) {
        ...
        rooms.forEach(function (r, ri) {
            r.huespedes.forEach(function (h, hi) {
                if (!(ri === roomIdx && hi === guestIdx)) arr.push(h);
            });
        });
        return arr;
    }
    ...
    function mostrarErrores(prefix, e) {
        Object.keys(SUF).forEach(function (k) {
            ...
            if (input) {
                if (e[k]) input.classList.add("invalido"); else input.classList.remove("invalido");
            }
        });
    }
    ...
    function limpiarErroresHab() {
        document.querySelectorAll(".cin-err,.cout-err").forEach(function (s) {
            s.textContent = "";
        });
        document.querySelectorAll(".cin,.cout").forEach(function (i) {
    ...
    fetch("/recepcionista/api/habitaciones")
        ...
        .then(function (data) {
            disponibles = data;
            ["cli-numdoc", "hu-numdoc"].forEach(function (id) {
                soloDigitos(id, 8);
            });
            ["cli-telefono", "hu-telefono"].forEach(function (id) {
                soloDigitos(id, 9);
            });
    ...
    function cargarDetalle(codigo) {
        fetch("/recepcionista/api/crud/r/" + encodeURIComponent(codigo))
            ...
            .then(function (d) {
                var c = d.cliente || {};
                setVal("cli-nombres", c.nombres);
                setVal("cli-apellidos", c.apellidos);
                setVal("cli-fnac", c.fechaNacimiento);
                setVal("cli-direccion", c.direccion);
                setVal("cli-tipodoc", c.idTipoDocumento);
                setVal("cli-numdoc", c.numeroDocumento);
                setVal("cli-correo", c.correo);
                setVal("cli-telefono", c.telefono);
    ...
    function opcionesHab(sel) {
        return disponibles.map(function (h) {
            return '<option value="' + h.id + '"' + (h.id === sel ? ' selected' : '') + '>Habitacion → ' + h.numero + '</option>';
        }).join("");
    }

    function caracteristicas(id) {
        var h = habDe(id);
        return h ? (h.tipo + ' · cap ' + h.capacidad + ' · ' + (h.descripcion || "")) : "";
    }
    ...
    function render() {
        ...
        rooms.forEach(function (room, idx) {
            ...
            div.querySelector(".cin").onchange = function () {
                room.checkIn = this.value;
            };
            div.querySelector(".cout").onchange = function () {
                room.checkOut = this.value;
            };
            div.querySelector(".del-room").onclick = function () {
                if (rooms.length <= 1) {
                    alert("La reserva debe tener al menos una habitación.");
                    return;
                }
                rooms.splice(idx, 1);
                render();
            };
            var ar = div.querySelector(".add-room");
            if (ar) ar.onclick = function () {
                rooms.push(nuevaRoom());
                render();
            };
            div.querySelector(".add-hue").onclick = function () {
                abrirModal(idx, -1);
            };
            div.querySelectorAll(".mini-edit").forEach(function (b) {
                b.onclick = function () {
                    abrirModal(idx, +b.dataset.h);
                };
            });
            div.querySelectorAll(".mini-del").forEach(function (b) {
                b.onclick = function () {
                    room.huespedes.splice(+b.dataset.h, 1);
                    render();
                };
            });
    ...
    function abrirModal(roomIdx, editIdx) {
        ...
        setVal("hu-nombres", g.nombres);
        setVal("hu-apellidos", g.apellidos);
        setVal("hu-fnac", g.fechaNacimiento);
        setVal("hu-direccion", g.direccion);
        setVal("hu-tipodoc", g.idTipoDocumento);
        setVal("hu-numdoc", g.numeroDocumento);
        setVal("hu-correo", g.correo);
        setVal("hu-telefono", g.telefono);
        mostrarErrores("hu", {});
    ...
    document.getElementById("modal-agregar").onclick = function () {
        ...
        if (modalCtx.edit >= 0) {
            room.huespedes[modalCtx.edit] = nuevo;
        } else {
            if (room.huespedes.length >= cap) {
                alert("No se pueden agregar más huéspedes: la habitación admite máximo " + cap + ".");
                return;
            }
    ...
    document.getElementById("btn-buscar-cli").onclick = function () {
        var doc = val("cli-numdoc");
        if (!doc) {
            alert("Ingresa el N° de documento.");
            return;
        }
        fetch("/recepcionista/api/crud/buscar?doc=" + encodeURIComponent(doc))
            ...
            .then(function (res) {
                if (!res.found) {
                    alert("No se encontró un cliente con ese documento. Completa los datos para crearlo.");
                    return;
                }
                var p = res.persona;
                setVal("cli-nombres", p.nombres);
                setVal("cli-apellidos", p.apellidos);
                setVal("cli-fnac", p.fechaNacimiento);
                setVal("cli-direccion", p.direccion);
                setVal("cli-tipodoc", p.idTipoDocumento);
                setVal("cli-correo", p.correo);
                setVal("cli-telefono", p.telefono);
                mostrarErrores("cli", {});
            });
    };
    ...
    document.getElementById("btn-guardar").onclick = function () {
        ...
        rooms.forEach(function (r) {
            r.huespedes.forEach(function (h) {
                listaHu.push(h);
            });
        });
        ...
        for (var i = 0; i < rooms.length; i++) {
            var r = rooms[i];
            if (!r.idHabitacion) {
                alert("Selecciona la habitación " + (i + 1) + ".");
                return;
            }
            if (r.huespedes.length > capDe(r.idHabitacion)) {
                alert("La habitación " + (i + 1) + " excede su capacidad.");
                return;
            }
        }
        ...
        var dto = {
            cliente: cliente, habitaciones: rooms.map(function (r) {
