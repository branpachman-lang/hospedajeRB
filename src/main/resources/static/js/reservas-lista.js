  ...
  function render(lista) {
    ...
    lista.forEach(function (r) {
      ...
      tr.innerHTML =
        ...
        '</div></td>';
      if (r.pagada) {
        tr.querySelector(".ic-print").onclick = function () { window.ComprobantePago.imprimir(r.codigo); };
        tr.querySelector(".btn-anular").onclick = function () { window.ComprobantePago.anular(r.codigo); };
      } else {
        tr.querySelector(".ic-del").onclick = function () { eliminar(r.codigo); };
        tr.querySelector(".btn-comp").onclick = function () { window.ComprobantePago.abrirModal(r.codigo); };
      }
      tb.appendChild(tr);
    });
  }
