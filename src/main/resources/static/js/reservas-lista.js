(function () {
    ...
    lista.forEach(function (r) {
      ...
      tr.innerHTML =
        ...
        '<td><div class="col-acciones">' +
          ...
          '<button class="ic-btn ic-del" title="Eliminar" ' + (r.pagada ? "disabled" : "") + '>✖</button>' +
          '<button class="btn-comp">Generar Comprobante</button>' +
        '</div></td>';
      var btnDel = tr.querySelector(".ic-del");
      if (!r.pagada) btnDel.onclick = function () { eliminar(r.codigo); };
      tr.querySelector(".btn-comp").onclick = function () { comprobante(r.codigo); };
      tb.appendChild(tr);
    });
  }
  ...
  function comprobante(codigo) {
    var h = {}; h[csrfHeader] = csrfToken;
    fetch("/recepcionista/api/crud/" + codigo + "/comprobante", { method: "POST", headers: h })
      .then(function (r) { return r.json(); })
      .then(function (res) { alert(res.mensaje || "Listo"); })
      .catch(function () { alert("Error al generar comprobante"); });
  }

  function leer(r) { return r.json().then(function (b) { return { ok: r.ok, body: b }; }); }
