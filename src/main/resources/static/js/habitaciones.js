(function () {
    ...
    function cargarHabitaciones() {
        ...
        fetch("/recepcionista/api/habitaciones" + (q.length ? "?" + q.join("&") : ""))
            ...
            .then(function (data) {
                ...
                if (pisos.indexOf(pisoSel) === -1) {
                    pisoSel = pisos.length ? pisos[0] : null;
                }
                renderPisos();
                renderLista();
                cargarReservas();
    ...
    function renderPisos() {
        ...
        pisosUnicos().forEach(function (p) {
            var n = habitaciones.filter(function (h) {
                return h.piso === p;
            }).length;
    ...
    function renderLista() {
        ...
        cont.innerHTML = "";
        var lista = habitaciones.filter(function (h) {
            return h.piso === pisoSel;
        });
        if (!lista.length) {
            cont.innerHTML = '<div class="cal__vacio">Sin habitaciones.</div>';
            return;
        }
    ...
    function renderCalendario() {
        ...
        if (!lista.length) {
            ...
            v.className = "cal__vacio";
            v.textContent = "Selecciona un piso con habitaciones.";
            cal.appendChild(v);
            return;
        }
    ...
    document.getElementById("sem-prev").onclick = function () {
        semanaInicio = addDias(semanaInicio, -7);
        cargarReservas();
    };
    document.getElementById("sem-next").onclick = function () {
        semanaInicio = addDias(semanaInicio, 7);
        cargarReservas();
    };
    document.getElementById("sem-hoy").onclick = function () {
        semanaInicio = domingoDe(new Date());
        cargarReservas();
    };
